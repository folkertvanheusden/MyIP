...
ax25::ax25(stats *const s, const any_addr & my_mac, const any_addr & my_ip, router *const r) : network_layer(s, "AX.25", r), my_mac(my_mac)
{
	assert(my_mac.get_family() == any_addr::ax25);
	assert(my_ip.get_family()  == any_addr::ipv4);

// TODO	ip4 = new ipv4(s, arp *const iarp, my_ip, r, 1);

	ax25_total   = s->register_stat("ax25_total",   "1.3.6.1.2.1.4.57850.1.14.1");
	ax25_invalid = s->register_stat("ax25_invalid", "1.3.6.1.2.1.4.57850.1.14.2");
...
void ax25::operator()()
{
	...
	while(!stop_flag) {
		...
		auto          ax25_payload = pkt->get_payload();

		stats_inc_counter(ax25_total);

		if (ax25_payload.second < 16) {
			DOLOG(ll_info, "AX.25: not an AX.25 packet - too small (size: %zu)\n", ax25_payload.second);
			delete pkt;
			stats_inc_counter(ax25_invalid);
			continue;
		}

		ax25_packet ap(std::vector<uint8_t>(ax25_payload.first, ax25_payload.first + ax25_payload.second));

		if (!ap.get_valid()) {
			DOLOG(ll_info, "AX.25: not an AX.25 packet\n");
			stats_inc_counter(ax25_invalid);
		}
		else if (ap.get_pid() != 0x07) {
			DOLOG(ll_info, "AX.25: not an IP-over-AX.25 packet\n");
		}
		else {
		}
