...
sip_register::sip_register(udp *const u, const std::string & upstream_sip_server, const std::string & upstream_sip_user, const std::string & upstream_sip_password, const any_addr & myip, const int myport) :
	u(u),
...
void sip_register::operator()()
{
	...
	out += "User-Agent: MyIP\r\n";
	out += "From: <sip:" + username + " @" + myip.to_str() + ">\r\n";
	...
	while(!stop_flag) {
		...
		for(int i=0; i<60 * 4 && !stop_flag; i++)
			myusleep(250 * 1000);
	}
}
